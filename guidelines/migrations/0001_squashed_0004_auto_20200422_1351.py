# Generated by Django 3.0.5 on 2020-04-22 15:21

import django.db.models.deletion
import wagtail.blocks
import wagtail.fields
from django.db import migrations, models


class Migration(migrations.Migration):
    initial = True

    dependencies = [
        ("modules", "0001_squashed_0002_auto_20200422_1348"),
    ]

    operations = [
        migrations.CreateModel(
            name="GuidelinesListingPage",
            fields=[
                (
                    "translatablepage_ptr",
                    models.IntegerField(
                        auto_created=True, db_column="translatablepage_ptr_id", primary_key=True, serialize=False
                    ),
                ),
                ("introduction", wagtail.fields.RichTextField(blank=True, default="")),
            ],
            options={
                "abstract": False,
            },
        ),
        migrations.CreateModel(
            name="GuidelinesSectionPage",
            fields=[
                (
                    "translatablepage_ptr",
                    models.IntegerField(
                        auto_created=True, db_column="translatablepage_ptr_id", primary_key=True, serialize=False
                    ),
                ),
                ("subtitle", models.CharField(max_length=140)),
                (
                    "section_colour",
                    models.CharField(
                        choices=[("#25a898", "Green"), ("#007fc2", "Blue"), ("#79b04a", "Lime"), ("#7f97ab", "Grey")],
                        max_length=140,
                    ),
                ),
                (
                    "landing_page_summary",
                    models.CharField(
                        help_text="Text to be shown on the guidelines landing page", max_length=240, null=True
                    ),
                ),
                (
                    "body",
                    wagtail.fields.StreamField(
                        [
                            (
                                "content",
                                wagtail.blocks.StructBlock(
                                    [
                                        (
                                            "title",
                                            wagtail.blocks.CharBlock(
                                                help_text="Section title, max length 120 characters", max_length=120
                                            ),
                                        ),
                                        ("content", wagtail.blocks.RichTextBlock()),
                                    ]
                                ),
                            )
                        ],
                        blank=True,
                        null=True,
                    ),
                ),
                (
                    "sub_sections_title",
                    models.CharField(help_text="Title for child pages under this section", max_length=140),
                ),
            ],
            options={
                "abstract": False,
            },
        ),
        migrations.CreateModel(
            name="GuidelinePage",
            fields=[
                (
                    "translatablepage_ptr",
                    models.IntegerField(
                        auto_created=True, db_column="translatablepage_ptr_id", primary_key=True, serialize=False
                    ),
                ),
                (
                    "body",
                    wagtail.fields.StreamField(
                        [
                            (
                                "content_section",
                                wagtail.blocks.StructBlock(
                                    [
                                        (
                                            "title",
                                            wagtail.blocks.CharBlock(
                                                help_text="Section title, max length 120 characters", max_length=120
                                            ),
                                        ),
                                        ("content", wagtail.blocks.RichTextBlock()),
                                    ]
                                ),
                            ),
                            (
                                "dos_and_donts",
                                wagtail.blocks.StructBlock(
                                    [
                                        (
                                            "title",
                                            wagtail.blocks.CharBlock(
                                                help_text="Do's and dont's title", max_length=200
                                            ),
                                        ),
                                        (
                                            "dos",
                                            wagtail.blocks.ListBlock(
                                                wagtail.blocks.StructBlock(
                                                    [
                                                        (
                                                            "item",
                                                            wagtail.blocks.CharBlock(
                                                                help_text="Item text", max_length=250
                                                            ),
                                                        )
                                                    ]
                                                )
                                            ),
                                        ),
                                        (
                                            "donts",
                                            wagtail.blocks.ListBlock(
                                                wagtail.blocks.StructBlock(
                                                    [
                                                        (
                                                            "item",
                                                            wagtail.blocks.CharBlock(
                                                                help_text="Item text", max_length=250
                                                            ),
                                                        )
                                                    ]
                                                )
                                            ),
                                        ),
                                    ]
                                ),
                            ),
                        ],
                        blank=True,
                        null=True,
                    ),
                ),
                (
                    "links_module",
                    models.ForeignKey(
                        blank=True,
                        null=True,
                        on_delete=django.db.models.deletion.SET_NULL,
                        related_name="+",
                        to="modules.LinksModule",
                    ),
                ),
                (
                    "more_information_module",
                    models.ForeignKey(
                        blank=True,
                        null=True,
                        on_delete=django.db.models.deletion.SET_NULL,
                        related_name="+",
                        to="modules.MoreInformationModule",
                    ),
                ),
            ],
            options={
                "abstract": False,
            },
        ),
    ]
